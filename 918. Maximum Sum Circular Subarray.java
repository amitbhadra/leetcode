...
class Solution {
    public int maxSubarraySumCircular(int[] A) {
        int run_max=A[0], run_min=A[0], cur_max=A[0], cur_min=A[0], total=A[0];
        for(int i=1; i<A.length; i++) {
            run_max = Math.max(run_max+A[i], A[i]);
            cur_max = Math.max(cur_max, run_max);
            run_min = Math.min(run_min+A[i], A[i]);
            cur_min = Math.min(cur_min, run_min);
            total+=A[i];
        }
        if(cur_max>0)
            return Math.max(cur_max, total-cur_min);
        else return cur_max;
    }
}